...
-- and prompt_version for auditability
...
ALTER TABLE ai_product_analysis
  ...
  ADD COLUMN IF NOT EXISTS stylemate_status     text   NOT NULL DEFAULT 'pending'
    CHECK (stylemate_status IN ('pending','processing','completed','failed','needs_review'));

CREATE INDEX IF NOT EXISTS idx_ai_analysis_stylemate_status
  ON ai_product_analysis (tenant_id, stylemate_status);

CREATE INDEX IF NOT EXISTS idx_ai_analysis_product_tenant
  ON ai_product_analysis (product_id, tenant_id);
